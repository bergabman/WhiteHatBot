...
use serenity::http::UserPagination;
...
pub async fn howtohack(ctx: &Context, msg: &Message) -> CommandResult {
    ...
    if config.channel_ids.contains(&msg.channel_id) {
        ...
        let new_embed2 = CreateEmbed::new()
            ...
            .colour(Color::DARK_TEAL)
            .footer(new_footer);
        let message2_new = EditMessage::new().content("").embed(new_embed2);
        let _ = message2.edit(&ctx, message2_new).await;
...
#[command]
pub async fn bans(ctx: &Context, msg: &Message) -> CommandResult {
    let data = ctx.data.read().await;
    ...
    loop {
        ...
        if bans.len() == 1000 {
            last_user_id = Some(bans.last().clone().unwrap().user.id);
        } else {
            break
        }
    }

    msg.channel_id.say(&ctx.http, all_bans_count.to_string()).await?;
